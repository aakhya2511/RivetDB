...
func NewRebalanceCollector(meta *MetaRange, router *Router, c clock.Clock, alphaPPM uint64) (*RebalanceCollector, error) {
	if meta == nil || router == nil || c == nil {
	...
	return &RebalanceCollector{meta: meta, router: router, clock: c, sampler: sampler}, nil
}
...
func (c *RebalanceCollector) Collect(ctx context.Context) (RebalanceClusterSnapshot, error) {
	...
	for _, nodeID := range bootstrap.Nodes {
		metric := &RebalanceNodeMetric{NodeID: nodeID, Available: nodes[nodeID] != nil, Healthy: nodes[nodeID] != nil}
	...
	for _, descriptor := range bootstrap.Ranges {
		...
		for _, placement := range descriptor.Replicas {
			...
			readRate, writeRate, requestRate, samples, err := c.sampler.Observe(placement.ReplicaID, TrafficCounters{Reads: local.Reads, Writes: local.Writes, Requests: local.Requests})
			if err != nil {
